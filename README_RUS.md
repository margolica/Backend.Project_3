...
```
URL-адрес проксируемого сервера устанавливается с помощью директивы proxy_pass и может использовать HTTP или HTTPS в качестве протокола, доменного имени или IP-адреса, а также необязательного порта и URI в качестве адреса.
...
- gzip_disable "msie6" — исключает IE6 из браузеров, которые будут получать сжатые файлы (не поддерживает GZIP).
...
- gzip_proxied — сжимает данные ответов для proxy-серверов;
...
- gzip_comp_level 6 — устанавливает, сколько файлов будет сжато. Чем выше число, тем выше уровень сжатия и использование ресурсов: 1 — минимальное, 9 — максимальное.

- gzip_http_version 1.1 — используется для ограничения сжатия gzip для браузеров, поддерживающих протокол HTTP/1.1. Если браузер не поддерживает его, вероятно, что он не поддерживает и gzip.

- gzip_min_length 1100 — сообщает Nginx не сжимать файлы размером менее 256 байт.
...
**NGINX 1.13.9**, выпущенный 20 февраля 2018 года, включает поддержку HTTP/2-проталкивания на сервер. Чтобы отсылать ресурсы вместе с загрузкой страницы, используй http2_push директиву следующим образом:
